 ...
 */
function BotrUpload(link, resumableSession, redirect, id) {
  // Whether we are in resumable mode
  this._resumable = !!resumableSession;
  
  if(this._resumable && !BotrUpload.resumeSupported()) {
  ...
  if(redirect) {
    ...
    if(redirect.params) {
      for(p in redirect.params) {
        this._redirectableUrl += '&redirect_query.' + encodeURI(p)
          + '=' + encodeURI(redirect.params[p]);
      }
    }
  }
  ...
  // the upload progress.
  if(!this._resumable) {
    this._progressUrl = link.protocol + '://' + link.address
      + '/progress?token=' + link.query.token
      + '&callback=' + id + '_poll';
  }
}

BotrUpload.prototype = {
  constructor: BotrUpload,
  ...
  getForm: function() {
    if(!this._form) {
      ...
      {
        var div = document.createElement('div');
        div.innerHTML = '<form method="post" enctype="multipart/form-data"></form>';
        f = div.firstChild;
        div.removeChild(f);
  ...
  _attachEvent: function(element, event, callback, self) {
    var cb = callback;
    if(self) {
      cb = function() { callback.call(self); };
    }
 ...
 */
BotrUpload.resumeSupported = function() {
  return (
