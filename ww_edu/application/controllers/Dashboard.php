...
class Dashboard extends MY_Controller
{
	public function previews_since_today()
	{
		...
		$last_trading_count = $this->trading_datas_calculate->build($mt4, $last_month)->count()->property('get_count', [])->get_property();
		$abs = $data['data']['trading_count'] - $last_trading_count;
		$desc = ($abs > 0) ? ' Higher' : ' Lower';
		$data['data']['last_trading_count'] = abs(round(abs($abs) / $last_trading_count, 3)) * 100 . '%' . $desc;
		...
		$last_profit = $this->trading_datas_calculate->build($mt4, $last_month)->count()->property('sum', ['profit'])->get_property();
		$abs = $data['data']['profit'] - $last_profit;
		$desc = ($abs > 0) ? ' Higher' : ' Lower';
		$data['data']['last_profit'] = abs(round(abs($abs) / $last_profit, 3)) * 100 . '%' . $desc;
		...
		$last_avg_holding_time = $this->trading_datas_calculate->build($mt4, $last_month)->count()->property('avg_deviation', ['order_open_time', 'order_close_time'])->get_property();
		...
		$last_transaction_peroid = $this->trading_datas_calculate->build($mt4, $last_month)->count()->property('transaction_peroid', ['order_open_time'])->get_property();
