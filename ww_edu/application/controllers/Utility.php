...
class Utility extends MY_Controller
{
	public function week_news()
	{
		...
		$data['data']['news'] = $this->trading_datas_calculate->build($mt4)->property('setUnixTime', [$start, $nextOrLast, $time_node])->get_week()->getWeekResult();
	...
	public function addTradingLog()
	{
		...
		$cols = $this->trading_log->init($datas)->create_time()->format()->property('user_addslashes',[])->pickUpProperty()->add();
