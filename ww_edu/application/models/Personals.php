...
class Personals extends CI_Model 
{
	...
	function show_mail($id, $is_read, &$data)
	{
		$map = 'SELECT m.*,c.title, me.face AS from_face
				FROM mail_from_to m
				LEFT JOIN comment c 
				ON c.dis_id=m.dis_id 
				LEFT JOIN member me 
				ON me.id=m.from_id
				WHERE m.mem_id="'.$id.'" 
				AND is_read='.$is_read.' 
				AND m.dis_id IS NOT NULL
				UNION
				SELECT m.*,c.name AS title, me.face AS from_face
				FROM mail_from_to m
				LEFT JOIN classes_text c 
	...
	function isexists_inner_mail($mem_id, &$data)
	{
		$map = 'SELECT count(*) as count 
				FROM mail_for_mem
				WHERE mem_id="'.$mem_id.'"';
		...
		$result = $this->db->query($map)->row_array();
		$data['data']['mail_is_exists'] = $result?'EXIST':'NONE';
	}

	function mail_count($mem_id,$is_read)
	{
		$map = 'SELECT count(*) as count 
				FROM mail_for_mem
				WHERE mem_id="'.$mem_id.'"
	...
	function is_read($id, $is_read=1)
	{
		$map = 'UPDATE mail_for_mem 
				SET is_read="'.$is_read.'" 
				WHERE id="'.$id.'"';
