...
class Dashboard extends MY_Controller
{
	public function previews_since_today()
	{
		...
		$data['data']['last_trading_count'] = abs(round(abs($abs) / $last_trading_count, 2)) * 100 . '%' . $desc;
		...
		$desc = ($abs > 0) ? ' Higher' : ' Lower';
		$data['data']['last_profit'] = abs(round(abs($abs) / $last_profit, 2)) * 100 . '%' . $desc;
		...
		$desc = ($abs > 0) ? ' Higher' : ' Lower';
		$data['data']['last_avg_holding_time'] = abs(round(abs($abs) / $last_avg_holding_time, 2)) * 100 . '%' . $desc;
		...
		$data['data']['last_transaction_peroid'] = abs(round(abs($abs) / $last_transaction_peroid, 2)) * 100 . '%' . $desc;
