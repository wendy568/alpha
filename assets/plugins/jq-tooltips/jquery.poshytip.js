...
(function($) {
	...
	$.Poshytip = function(elm, options) {
		...
		var arrow = this.opts.alignX == 'left' ? 'right' : this.opts.alignX == 'right' ? 'left' : this.opts.alignY == 'bottom' ? 'top' : 'bottom';
		this.$tip = $(['<div class="',this.opts.className,'">',
				'<div class="tip-inner tip-bg-image"></div>',
				'<div class="tip-arrow tip-arrow-"'+arrow+'></div>',
			'</div>'].join(''));
	...
	$.fn.poshytip = function(options){
		...
		if (!$('#poshytip-css-' + opts.className)[0])
			$(['<style id="poshytip-css-',opts.className,'" type="text/css">',
				...
				'div.',opts.className,' div.tip-arrow{position:absolute;overflow:hidden;font:1px/1px sans-serif;}',
